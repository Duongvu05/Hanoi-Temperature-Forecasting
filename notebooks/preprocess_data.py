...
def handle_sklearn_compatibility():
    ...
    try:
        # For newer sklearn versions, we need to patch the missing attribute
        if not hasattr(sklearn.compose._column_transformer, '_RemainderColsList'):
            # Create a simple dummy class for compatibility
...
def get_model_paths():
    ...
    try:
        # Try to get the current directory (where this script is located)
        current_dir = Path(__file__).parent
        model_file = current_dir / "BEST_CATBOOST_TIMESERIES.joblib"
        selection_file = current_dir / "selection_result.joblib"
...
def process_data(df: pd.DataFrame):
    ...
    df["pressure_trend_7d"] = df["sealevelpressure"] - df["sealevelpressure"].shift(14)
    df["pressure_trend_7d"] = df["sealevelpressure"] - df["sealevelpressure"].shift(21)
...
def select_top_k(X, k=93):
    ...
    try:
        ...
        if selection_path is None or not selection_path.exists():
            ...
            return X.iloc[:, :k] if len(X.columns) > k else X
...
def predict_future(df_raw: pd.DataFrame) -> pd.DataFrame:
    """
    Predict future temperatures with comprehensive error handling
    """
